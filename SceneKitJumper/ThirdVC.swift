...
class ThirdVC: UIViewController, SCNPhysicsContactDelegate {
    ...
    func creatView() {
        ...
        //创建view
            ...
            let constraintH = NSLayoutConstraint.constraints(withVisualFormat: "H:|-0-[view]-0-|", options: NSLayoutConstraint.FormatOptions(rawValue: 0), metrics: nil, views: ["view": scnView])
            let constraintV = NSLayoutConstraint.constraints(withVisualFormat: "V:|-0-[view]-0-|", options: NSLayoutConstraint.FormatOptions(rawValue: 0), metrics: nil, views: ["view": scnView])
        ...
        //创建地板：（用于光影效果，与落地判断）
            ...
            scnfloor.firstMaterial?.diffuse.contents = UIColor.white
            floor.geometry = scnfloor
            let floorbody = SCNPhysicsBody.static()
            floorbody.restitution = 0
            floorbody.friction = 1
            floorbody.damping = 0.3
            floorbody.categoryBitMask = NS_OPTIONS.CollisionDetectionMaskFloor.value()
            floorbody.collisionBitMask = NS_OPTIONS.CollisionDetectionMaskJumper.value()|NS_OPTIONS.CollisionDetectionMaskPlatform.value()|NS_OPTIONS.CollisionDetectionMaskOldPlatform.value();
            floorbody.contactTestBitMask = NS_OPTIONS.CollisionDetectionMaskJumper.value()
        ...
        //初始化小人，小人是动态物体，自由落体到第一个台子中心，会受重力影响，会与台子和地板碰撞
            ...
            jumper.geometry = box
            let jumperbody = SCNPhysicsBody.dynamic()
            jumperbody.restitution = 0
            jumperbody.friction = 1
            jumperbody.rollingFriction = 1
            jumperbody.damping = 0.3
            jumperbody.allowsResting = true
            jumperbody.categoryBitMask = NS_OPTIONS.CollisionDetectionMaskJumper.value();
            jumperbody.collisionBitMask = NS_OPTIONS.CollisionDetectionMaskPlatform.value()|NS_OPTIONS.CollisionDetectionMaskFloor.value()|NS_OPTIONS.CollisionDetectionMaskOldPlatform.value();
            jumper.physicsBody = jumperbody
            jumper.position = SCNVector3Make(0, 12.5, 0)
            self.scene.rootNode.addChildNode(jumper)
        ...
        //初始化相机：光源随相机移动，所以将光源设置为相机的字节点
            ...
            camera.eulerAngles = SCNVector3Make(-0.7, 0.6, 0)
            camera.addChildNode(self.light)


        //灯光
            let scnlight = SCNLight()
            scnlight.color = UIColor.white
            scnlight.type = SCNLight.LightType.omni
    ...
    @objc func reStart() {
        ...
        nextPlatform.removeFromParentNode()
        self.score = 0;
